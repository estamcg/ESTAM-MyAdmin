...
try {
  ...
  $stmt = $pdo->prepare("INSERT INTO filiere (secteur_key, acro, label) VALUES (?, ?, ?)");
  $stmt->execute([$secteurKey, $acro, $label]);

  respond('success', 'Filière ajoutée.', 201, [
    'id' => (int)$pdo->lastInsertId()
  ]);
} catch (Throwable $e) {
  // Unique constraint (acro)
